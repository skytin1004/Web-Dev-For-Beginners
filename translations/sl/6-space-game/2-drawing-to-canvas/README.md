...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T23:46:15+00:00",
...
Platno je HTML element, ki privzeto nima vsebine; je prazno platno. Dodati mu morate vsebino z risanjem nanj.

✅ Preberite [več o Canvas API](https://developer.mozilla.org/docs/Web/API/Canvas_API) na MDN.

Tako je običajno deklarirano kot del telesa strani:
...
Platno uporablja kartezični koordinatni sistem za risanje. Zato uporablja x-os in y-os za izražanje lokacije nečesa. Lokacija `0,0` je zgornji levi kot, spodnji desni kot pa je tisto, kar ste določili kot ŠIRINO in VIŠINO platna.
...
2. **Pridobite referenco** na element Context, ki se nahaja na elementu platna.
...
- **Slike**, lahko narišete sliko na podlagi slikovne datoteke, na primer .jpg ali .png.
...
Priporočljivo je, da zgornje zavijete v konstrukcijo, kot je ta, da je lažje za uporabo in da poskrbite, da ga poskušate manipulirati šele, ko je popolnoma naložen:
...
Odprite kopijo te mape v Visual Studio Code. Potrebujete lokalno razvojno okolje, po možnosti z Visual Studio Code, NPM in Node nameščenimi. Če na svojem računalniku nimate nastavljenega `npm`, [tukaj je, kako to storiti](https://www.npmjs.com/get-npm).
...
Zgornje bo zagnalo HTTP strežnik na naslovu `http://localhost:5000`. Odprite brskalnik in vnesite ta naslov. Trenutno je stran prazna, vendar se bo to spremenilo.
...
2. **Naložite** teksture  
   > namig: dodajte slike igralca in sovražnika z uporabo `await loadTexture` in podajanjem poti do slike. Na zaslonu jih še ne boste videli!
...
Naučite se več o Canvas API z [branjem o njem](https://developer.mozilla.org/docs/Web/API/Canvas_API).
