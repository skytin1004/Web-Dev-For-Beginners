...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T23:40:10+00:00",
...
Kako onda pomičemo stvari na ekranu? Sve se svodi na kartezijanske koordinate: mijenjamo lokaciju (x, y) objekta i zatim ponovno crtamo ekran.
...
2. **Očistite ekran**, ekran treba očistiti između crtanja. Možemo ga očistiti tako da nacrtamo pravokutnik ispunjen bojom pozadine.
3. **Ponovno nacrtajte objekt** na novoj lokaciji. Time konačno postižemo pomicanje objekta s jedne lokacije na drugu.
...
✅ Možete li smisliti razlog zašto ponovno crtanje vašeg heroja mnogo puta u sekundi može uzrokovati probleme s performansama? Pročitajte o [alternativama ovom obrascu](https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas).

## Rukovanje događajima tipkovnice

Događajima upravljate tako da povežete specifične događaje s kodom. Događaji tipkovnice aktiviraju se na cijelom prozoru, dok se događaji miša poput `click` mogu povezati s klikom na određeni element. U ovom projektu koristit ćemo događaje tipkovnice.
...
Gornji kod osigurat će da tipke sa strelicama i razmaknica imaju isključeno *zadano* ponašanje. Mehanizam *isključivanja* događa se kada pozovemo `e.preventDefault()`.

## Kretanje izazvano igrom

Možemo učiniti da se stvari same kreću pomoću timera kao što su funkcije `setTimeout()` ili `setInterval()` koje ažuriraju lokaciju objekta pri svakom taktu ili vremenskom intervalu. Evo kako to može izgledati:
...
Gornja petlja poziva se svakih `200` milisekundi kako bi ponovno nacrtala platno. Možete odabrati najbolji interval koji ima smisla za vašu igru.
...
- **Pomicanje neprijatelja**: također ćete trebati dodati kod kako biste osigurali da se neprijatelji kreću od vrha prema dnu određenom brzinom.
...
1. **Dodajte namjenske objekte** za `hero`, `enemy` i `game object`, oni bi trebali imati svojstva `x` i `y`. (Sjetite se dijela o [Nasljeđivanju ili kompoziciji](../README.md)).
   ...
   *SAVJET* `game object` bi trebao biti onaj s `x` i `y` i mogućnošću crtanja na platno.
   ...
   >savjet: započnite dodavanjem nove klase GameObject s konstruktorom kao što je dolje, a zatim je nacrtajte na platno:
...
2. **Dodajte rukovatelje događajima tipki** za upravljanje navigacijom tipki (pomicanje heroja gore/dolje lijevo/desno)
   ...
   *ZAPAMTITE* to je kartezijanski sustav, gornji lijevi kut je `0,0`. Također zapamtite dodati kod za zaustavljanje *zadanog ponašanja*.
   ...
   Provjerite konzolu preglednika u ovom trenutku i promatrajte kako se pritisci tipki bilježe. 

3. **Implementirajte** [Pub-sub obrazac](../README.md), ovo će održati vaš kod čistim dok pratite preostale dijelove.

   Da biste dovršili ovaj posljednji dio, možete:

   1. **Dodajte osluškivač događaja** na prozor:
...
1. **Postavite petlju igre**
   ...
   Refaktorirajte funkciju window.onload kako biste inicijalizirali igru i postavili petlju igre na odgovarajući interval. Također ćete dodati laserski snop:
...
Kao što vidite, vaš se kod može pretvoriti u 'spaghetti kod' kada počnete dodavati funkcije, varijable i klase. Kako možete bolje organizirati svoj kod kako bi bio čitljiviji? Skicirajte sustav za organizaciju koda, čak i ako još uvijek ostaje u jednoj datoteci.
...
Iako pišemo svoju igru bez korištenja okvira, postoji mnogo okvira za razvoj igara temeljenih na JavaScript platnu. Odvojite malo vremena za [istraživanje o njima](https://github.com/collections/javascript-game-engines).
