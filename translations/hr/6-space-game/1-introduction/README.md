...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T23:49:08+00:00",
...
U igri imate `objekte igre`, koji su objekti prisutni na ekranu. To znači da imaju lokaciju u kartezijanskom koordinatnom sustavu, definiranu `x` i `y` koordinatama. Dok razvijate igru, primijetit ćete da svi vaši objekti igre imaju standardna svojstva, zajednička za svaku igru koju kreirate, a to su elementi koji su:

- **temeljeni na lokaciji** Većina, ako ne i svi, elementi igre temelje se na lokaciji. To znači da imaju lokaciju, `x` i `y`.
- **pokretni** Ovo su objekti koji se mogu premjestiti na novu lokaciju. To su obično junaci, čudovišta ili NPC-ovi (likovi koji nisu igrači), ali ne, na primjer, statični objekti poput stabala.
...
- **s odgodom** 'Odgoda' je tipično svojstvo kratkotrajnih objekata. Tipičan primjer je komad teksta ili grafički efekt poput eksplozije koji bi trebao biti vidljiv samo nekoliko milisekundi.

✅ Razmislite o igri poput Pac-Mana. Možete li identificirati četiri gore navedena tipa objekata u ovoj igri?
...
✅ Odvojite nekoliko minuta i zamislite kako bi junak iz Pac-Mana (npr. Inky, Pinky ili Blinky) bio napisan u JavaScriptu.

**Kompozicija**

Drugi način upravljanja nasljeđivanjem objekata je korištenje *kompozicije*. Tada objekti izražavaju svoje ponašanje ovako:
...
Ovaj obrazac se bavi idejom da različiti dijelovi vaše aplikacije ne bi trebali znati jedni za druge. Zašto? To olakšava razumijevanje što se općenito događa ako su različiti dijelovi odvojeni. Također olakšava nagle promjene ponašanja ako je to potrebno. Kako to postići? To radimo uspostavljanjem nekih koncepata:

- **poruka**: Poruka je obično tekstualni niz popraćen opcionalnim sadržajem (komadom podataka koji pojašnjava o čemu se radi u poruci). Tipična poruka u igri može biti `KEY_PRESSED_ENTER`.
...
Gore povezujemo događaj s tipkovnice, `ArrowLeft`, i šaljemo poruku `HERO_MOVE_LEFT`. Slušamo tu poruku i kao rezultat pomičemo `heroja`. Snaga ovog obrasca je u tome što event listener i heroj ne znaju jedan za drugog. Možete premapirati `ArrowLeft` na tipku `A`. Osim toga, moguće je učiniti nešto potpuno drugačije na `ArrowLeft` uz nekoliko izmjena u funkciji `on` eventEmittera:
...
Razmislite o tome kako pub-sub obrazac može unaprijediti igru. Koji dijelovi bi trebali emitirati događaje i kako bi igra trebala reagirati na njih? Sada je vaša prilika da budete kreativni, osmislite novu igru i kako bi se njeni dijelovi mogli ponašati.
