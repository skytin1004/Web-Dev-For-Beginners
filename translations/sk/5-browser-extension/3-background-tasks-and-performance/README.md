...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T23:29:20+00:00",
...
Téma, ako urobiť vaše webové stránky extrémne rýchle na všetkých typoch zariadení, pre všetkých typov používateľov a v rôznych situáciách, je pochopiteľne rozsiahla. Tu je niekoľko bodov, ktoré by ste mali mať na pamäti pri vytváraní štandardného webového projektu alebo rozšírenia prehliadača.

Prvá vec, ktorú musíte urobiť, aby ste zabezpečili efektívny chod vašej stránky, je zhromaždiť údaje o jej výkone. Prvým miestom na to sú vývojárske nástroje vášho webového prehliadača. V Edge môžete vybrať tlačidlo "Nastavenia a ďalšie" (ikona troch bodiek v pravom hornom rohu prehliadača), potom prejsť na Viac nástrojov > Vývojárske nástroje a otvoriť kartu Výkon. Klávesové skratky `Ctrl` + `Shift` + `I` na Windows alebo `Option` + `Command` + `I` na Macu tiež otvoria vývojárske nástroje.
...
✅ Spoznajte svoj profiler! Otvorte vývojárske nástroje na tejto stránke a zistite, či existujú nejaké úzke miesta. Ktorý aktívum sa načítava najpomalšie? Najrýchlejšie?
...
**Prechádzanie DOM**: Prehliadač musí vytvoriť svoj model objektov dokumentu na základe kódu, ktorý napíšete, takže v záujme dobrého výkonu stránky je vhodné udržiavať vaše značky minimálne, používať a štýlovať len to, čo stránka potrebuje. Napríklad nadbytočné CSS spojené so stránkou by mohlo byť optimalizované; štýly, ktoré sa majú používať len na jednej stránke, nemusia byť zahrnuté v hlavnom štýle.

**JavaScript**: Každý vývojár JavaScriptu by si mal dávať pozor na skripty, ktoré blokujú renderovanie a musia byť načítané pred tým, ako môže byť zvyšok DOM prechádzaný a vykreslený v prehliadači. Zvážte použitie `defer` s vašimi inline skriptami (ako je to urobené v module Terrarium).
...
Teraz, keď máte predstavu o tom, ako prehliadač vykresľuje aktíva, ktoré mu posielate, pozrime sa na posledné veci, ktoré musíte urobiť, aby ste dokončili svoje rozšírenie:
...
Chrome.runtime má [API](https://developer.chrome.com/extensions/runtime), ktoré spravuje všetky druhy úloh na pozadí, a vaše rozšírenie ho využíva:
...
Preskúmajte niektoré spôsoby, akými prehliadače hodnotia výkon webu, prezeraním kariet výkonu vo svojich webových nástrojoch. Nájdete nejaké zásadné rozdiely?
