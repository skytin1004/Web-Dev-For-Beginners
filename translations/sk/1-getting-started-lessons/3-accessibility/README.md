...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-28T00:17:12+00:00",
...
[Čítačky obrazovky](https://en.wikipedia.org/wiki/Screen_reader) sú bežne používané klientmi so zrakovým postihnutím. Tak ako venujeme čas zabezpečeniu, aby prehliadač správne sprostredkoval informácie, ktoré chceme zdieľať, musíme tiež zabezpečiť, aby čítačka obrazovky robila to isté.

V základnej podobe čítačka obrazovky číta stránku odhora nadol nahlas. Ak je vaša stránka čisto textová, čítačka sprostredkuje informácie podobným spôsobom ako prehliadač. Samozrejme, webové stránky zriedka obsahujú iba text; obsahujú odkazy, grafiku, farby a ďalšie vizuálne prvky. Je potrebné dbať na to, aby tieto informácie boli čítačkou obrazovky správne interpretované.
...
✅ Otestujte webovú stránku, ktorú radi používate, na použitie farieb pomocou rozšírenia prehliadača, ako je [kontrola kontrastu WCAG](https://microsoftedge.microsoft.com/addons/detail/wcag-color-contrast-check/idahaggnlnekelhgplklhfpchbfdmkjp?hl=en-US&WT.mc_id=academic-77807-sagibbon). Čo ste sa naučili?
...
Hypertextové odkazy sú jadrom navigácie na webe. Preto zabezpečenie, aby čítačka obrazovky mohla správne čítať odkazy, umožňuje všetkým používateľom navigovať na vašej stránke.

### Čítačky obrazovky a odkazy

Ako by ste očakávali, čítačky obrazovky čítajú text odkazov rovnakým spôsobom, ako by čítali akýkoľvek iný text na stránke. S týmto na pamäti by text uvedený nižšie mohol pôsobiť úplne prijateľne.

> Malý tučniak, niekedy známy ako rozprávkový tučniak, je najmenší tučniak na svete. [Kliknite sem](https://en.wikipedia.org/wiki/Little_penguin) pre viac informácií.

> Malý tučniak, niekedy známy ako rozprávkový tučniak, je najmenší tučniak na svete. Navštívte https://en.wikipedia.org/wiki/Little_penguin pre viac informácií.
...
Čítačky obrazovky majú tiež schopnosť čítať iba hypertextové odkazy na stránke, podobne ako by si vidiaci človek prehliadal stránku a hľadal odkazy. Ak je text odkazu vždy "kliknite sem", všetko, čo používateľ bude počuť, je "kliknite sem, kliknite sem, kliknite sem, kliknite sem, kliknite sem, ..." Všetky odkazy sú teraz nerozoznateľné.
...
> [Malý tučniak](https://en.wikipedia.org/wiki/Little_penguin), niekedy známy ako rozprávkový tučniak, je najmenší tučniak na svete.
...
Je samozrejmé, že čítačky obrazovky nie sú schopné automaticky čítať, čo je na obrázku. Zabezpečenie prístupnosti obrázkov nevyžaduje veľa práce - práve na to slúži atribút `alt`. Všetky významné obrázky by mali mať `alt`, ktorý popisuje, čo predstavujú. Obrázky, ktoré sú čisto dekoratívne, by mali mať svoj atribút `alt` nastavený na prázdny reťazec: `alt=""`. Tým sa zabráni tomu, aby čítačky obrazovky zbytočne oznamovali dekoratívny obrázok.

✅ Ako by ste očakávali, vyhľadávače tiež nie sú schopné pochopiť, čo je na obrázku. Používajú tiež text alt. Takže opäť, zabezpečenie prístupnosti vašej stránky prináša ďalšie bonusy!

## Klávesnica

Niektorí používatelia nie sú schopní používať myš alebo trackpad, namiesto toho sa spoliehajú na interakcie s klávesnicou, aby prechádzali z jedného prvku na druhý. Je dôležité, aby vaša webová stránka prezentovala váš obsah v logickom poradí, aby používateľ klávesnice mohol pristupovať ku každému interaktívnemu prvku, keď sa posúva po dokumente. Ak vytvárate svoje webové stránky so sémantickým ozna
